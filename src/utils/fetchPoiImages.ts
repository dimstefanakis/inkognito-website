...
async function getGooglePlacePhotos(placeId: string): Promise<ImageSource[]> {
  try {
    ...
    return details.result.photos.slice(0, 3).map((photo: { photo_reference: string, html_attributions: string[] }) => ({
...
async function getWikipediaImage(osmTags: {
  wikipedia: string;
}): Promise<ImageSource[]> {
...
async function getMapillaryImages(lat: number, lon: number): Promise<ImageSource[]> {
  ...
  try {
    ...
    return data.data.map((image: { thumb_1024_url: string }) => ({
